...
-- SELECT film_id, title, description, length, rental_rate FROM film WHERE title = "The Devil's Carnival";
...
-- SELECT title, rental_rate FROM film ORDER BY rental_rate OFFSET 10 FETCH NEXT 10 ROWS ONLY;	
...
-- SELECT staff.staff_id, ct.customer_id, ct.first_name, ct.last_name, pm.payment_date, pm.amount
-- FROM staff
-- LEFT JOIN payment pm ON staff.staff_id = pm.staff_id
-- LEFT JOIN customer ct ON pm.customer_id = ct.customer_id
